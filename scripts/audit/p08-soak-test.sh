...
stage_init p08 "Soak Test"
require_env BASE_URL "staging deployment authorised for load testing"
require_cmd npx "node package runner for autocannon"
if [ "${AUDIT_ALLOW_HEAVY_LOAD:-0}" != "1" ] && [ "p08" != "p05" ]; then
  not_verified "heavy load stage skipped: set AUDIT_ALLOW_HEAVY_LOAD=1 to authorise sustained traffic against $BASE_URL"
fi
out="$ARTIFACT_DIR/p08-autocannon.json"
npx --yes autocannon -c 20 -d 900 -j "$BASE_URL" > "$out" 2>"$ARTIFACT_DIR/p08-autocannon.log" || not_verified "autocannon could not run"
artifact "artifacts/p08-autocannon.json"
