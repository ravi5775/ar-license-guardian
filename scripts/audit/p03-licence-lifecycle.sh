...
fp="audit-$(date +%s)"
act=$(curl -s --max-time 20 -X POST "${BASE_URL%/}/api/public/license/activate" \
  -H 'content-type: application/json' \
  -d "{\"licenseKey\":\"$TEST_LICENCE_KEY\",\"deploymentFingerprint\":\"$fp\",\"domain\":\"audit.local\"}" || echo '{}')
...
st=$(curl -s -o /dev/null -w '%{http_code}' --max-time 20 "${BASE_URL%/}/api/public/licence/status?fingerprint=$fp")
